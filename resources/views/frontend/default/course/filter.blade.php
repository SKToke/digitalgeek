<div class="sidebar">
        ...
        <div class="col-6 text-end">
            @if(count(request()->all()) > 0 || !empty($category_details))
                <a class="btn d-flex align-items-center float-end border py-2" href="{{route('courses')}}"><i
                        class="fi-rr-cross-circle me-2"></i>
                    <span>{{get_phrase('Clear')}} @if(isset($_GET) && count($_GET) > 0)
                            ({{count($_GET)}})
                        @endif</span></a>
            @endif
        </div>
    ...
    <form action="{{ route('courses', request()->route()->parameter('category')) }}" method="get"
        ...
        <!------------------- c_course_type filter start ------------------->
        <div class="widget">
            <h4 class="widget-title">{{ get_phrase('Course Type') }}</h4>
            <ul class="entry-widget">
                @foreach (['recorded', 'session', 'live'] as $c_course_type)
                    <li class="filter-item">
                        <div class="form-check">
                            <input class="form-check-input mt-0" type="radio" name="c_course_type"
                                   value="{{ $c_course_type }}" id="c-course-type-{{ $c_course_type }}"
                                   @if (request()->has('c_course_type') && request()->input('c_course_type') == $c_course_type) checked @endif />
                            <label class="form-check-label"
                                   for="c-course-type-{{ $c_course_type }}">{{ get_phrase(ucfirst($c_course_type)) }}</label>
                        </div>
                    </li>
                @endforeach
            </ul>
